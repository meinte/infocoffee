...
/*
* These filters act whenever a ui state change occurs, it returns proper data depending on filters set through the selection boxes
*/
...
export const filterRegions = (coffee,uistate) => {
  ...
  const toReturn = []
  coffee.forEach((data) => {
    const isPartOfCountry = (data.land == uistate.selectedCountry || uistate.selectedCountry == DEFAULT_STATE)
    const isPartOfVariety = (data.variety.indexOf(uistate.selectedVariety)>-1 || uistate.selectedVariety == DEFAULT_STATE)
...
export const filterCountries = (coffee,uistate) => {
  ...
  const toReturn = []
  coffee.forEach((data) => {
    const isPartOfRegion = (data.region == uistate.selectedRegion || uistate.selectedRegion == DEFAULT_STATE)
    const isPartOfVariety = (data.variety.indexOf(uistate.selectedVariety)>-1 || uistate.selectedVariety == DEFAULT_STATE)
